...
args <- list(temp$Year, temp$Fatalities)

do.call(group_by, args)

as.name(response)
eval(as.name(response))
eval(as.name(paste0("temp$", as.name(response))))

product <- temp %>% group_by(Year, get(response)) %>% 
  summarise(count = n()) %>% 
  ggplot(aes(x = Year, y = eval(as.name(response)))) + geom_line() +
...
# Group by Motivations
temp_data %>% mutate(Year <- as.numeric(Year)) %>% 
  group_by(Year, Cause) %>% 
  summarise(count = n()) %>% 
  ggplot(aes(x = Year, y = count,  fill = factor(Cause))) + 
  geom_bar(stat = "identity", position = "fill")

# Group by Shooting types
temp_data %>% mutate(Year <- as.numeric(Year)) %>% 
  group_by(Year, Shooting.Type) %>% 
  summarise(count = n()) %>% 
  ggplot(aes(x = Year, y = count,  fill = factor(Shooting.Type))) + 
  geom_bar(stat = "identity", position = "fill")
